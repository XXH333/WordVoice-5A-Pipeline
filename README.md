...
**WordVoice Data Pipeline** 是 **WordVoice** 数据集官方发布的字级声学标注流水线，用于自动构建**大规模、高质量、字级声学数据集**，服务于可控 LLM Text-to-Speech（TTS）研究。

不同于传统 Forced Alignment 仅提供时间戳，本项目融合了：

- 双模型时间戳对齐（Dual-model Alignment）
- 边界优化（Boundary Refinement）
- 多维韵律标注（Prosodic Annotation）

能够自动生成 **WordVoice-5A** 等高质量字级数据集。
...
| 🔊 Energy | 提取字内 Top 50% 帧级均值以表征音节核心能量 |
| 🎵 Pitch | 剔除首尾（掐头去尾保留核心 80%）后的字级核心 F0 |
