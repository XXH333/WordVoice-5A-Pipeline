...
# 音高归一化到 [-1, 1], 对应的log区间为 [-11.5, 0]
# 能量归一化到 [0, 1], 对应的log区间为 [3.9, 6.7]
# 语调包括 ['falt', 'rise', 'fall', 'peak', 'valley']
# 声学边界包括 ['b0', 'b1', 'b2', 'b3']
import torch
import torchcrepe
import librosa
...
# 多进程处理封装
def process_single_line_wrapper(args_tuple):
    """用于多进程提取的包装函数"""
    data, wav_folder = args_tuple
    
    # 按照原逻辑，空列表则跳过不返回内容
    if not data.get('mfa_words'):
        return False, f"skip {data['utt']}: []"

    wav = f"{wav_folder}/{data['audio_path']}"
    try:
        # 默认全部送入 cuda
        result = extract_f0_energy_crepe_words(wav, data['mfa_words'], log_f0=True, log_energy=True, device='cuda')
        utt_duration = result['utt_duration']
        torch.cuda.empty_cache()
        data['f0'] = result['f0_words'].tolist()
        data['eng'] = result['energy_words'].tolist()
        data['tone'] = result['tone_words'].tolist()
        data['bnd'] = result['boundary_words'].tolist()
        if 'qwen_words' in data:
            del data['qwen_words']
        ...
        data = {
            ...
            'duration': round(utt_duration, 2),
            **data
        }
        return True, json.dumps(data, ensure_ascii=False)
    except Exception as e:
        return False, f"⚠️ 处理 {data['utt']} 时出错: {e}. 已跳过标注"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--align_json_path", type=str, required=True, help="输入数据JSONL路径")
    parser.add_argument("--wav_folder", type=str, required=True, help="音频文件夹路径")
    parser.add_argument("--output_dsp_file", type=str, required=True, help="输出的优化后JSONL路径")
    parser.add_argument("--language", type=str, default="zh", help="语言类型 (zh/en)")
    parser.add_argument("--high_quality", action="store_true", help="是否进行高质量数据筛选")
    # ⚠️警告:由于包含了 torchcrepe 模型推理，消耗显存，workers 建议设为 4 或 8。如果显卡显存(24G+)，可适度调高。
    parser.add_argument("--workers", type=int, default=12, help="并发数量(注意控制显存)")
    args = parser.parse_args()

    print(args.high_quality, type(args.high_quality))
    ori_data = []
    with open(args.align_json_path, "r", encoding="utf-8") as f:
        for line in f:
            if line.strip():
                item = json.loads(line)
                if (args.high_quality==False) or (item['confidence']==True):
                    ori_data.append(item)
    print(f"✅ Loaded align data from {args.align_json_path}, total {len(ori_data)} items")
    # 断点续传：加载已处理的 utt，避免重复计算
    print(f"⏳ Checking for already processed utterances in {args.output_dsp_file}...")
    try:
        processed_utts = load_processed_utts(args.output_dsp_file)
    except Exception as e:
        print(f"加载已处理数据失败: {e}")
        processed_utts = set()
    raw_data = []
    ...
    print(f"✅ Total {len(ori_data)} items to process after filtering already processed utterances")
    task_args = [(data, args.wav_folder) for data in ori_data]
    print(f"正在处理数据... (使用 GPU 并发: {args.workers})")
    with open(args.output_dsp_file, "a", encoding="utf-8") as f_out:
        # spawn 模式进程池
        with mp.get_context('spawn').Pool(processes=args.workers) as pool:
            for is_success, result_content in tqdm(
                pool.imap_unordered(
                    process_single_line_wrapper,
                    task_args,
                    chunksize=5
                ),
                total=len(task_args)
